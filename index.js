...
client.on("ready", () => {
  ...
  client.user.setActivity(`Dm moi si tu veut récupéré ton reward ou pour posé une question !`);
});
...
function SendMessageTicket(message) {
  ...
  const embed = new RichEmbed()
      .setColor(0xCF40FA)
      .addField(`<:notify:874216618182451210>**Nouveau message de** ${message.author.username}`, message.content + " " + pp)
      .setTimestamp();
  c.send({
      embed: embed
  });
...
client.on("message", async message => {
  ...
  if (message.channel.type !== "dm") {
      if (message.channel.name.indexOf('ticket') > -1 && command != "close") {
          ...
          const embed = new RichEmbed()
              .setColor(0xCF40FA)
              .addField(`<a:hggggg:874214809023938590>Message de ${message.author.username}`, message.content)
              .setTimestamp();
          pq.send({
              embed: embed
          });
          ...
          if (command == "close" && message.channel.name.indexOf('ticket') > -1) {
              ...
              const embed = new RichEmbed()
                  .setColor(0xCF40FA)
                  .addField(`<:jhhhhhh:873969194666954753>**Staff**<:jhhhhhh:873969194666954753>`, "<:hgggggg:873970185604837376>Votre ticket a été fermé inutile de répondre à ce message")
                  .setTimestamp();
              pq.send({
                  embed: embed
              });
  ...
  if (message.channel.type === "dm") {
      ...
      server.createChannel(`ticket-${message.author.id}`, "text").then(c => {
          ...
          message.channel.send(`<a:certif:873964059207299195> Votre ticket a été transmit au staff.`);
          const embed = new RichEmbed()
              .setColor(0xCF40FA)
